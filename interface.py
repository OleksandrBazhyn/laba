...
def enter_conditions():
    try:
        ...
        for i in range(num_interface_conditions):
            tk.Label(root, text=f"Interface Condition {i + 1}:").grid(row=10 + num_boundary_conditions + i, column=0,
                                                                      sticky=tk.W)
            x1_entry = tk.Entry(root, width=10)
            x2_entry = tk.Entry(root, width=10)
            t_entry = tk.Entry(root, width=10)
...
clear_conditions_button.grid(row=9, column=4)

# Display selected and defined functions
tk.Label(root, text="Selected function: x1 ** 2 + x2 ** 2 + T; u(s): 5; v: [0,0]; k = 1").grid(row=0, column=3)
...
get_values_button = tk.Button(root, text="Get Values", command=get_values, bg="pale green")
